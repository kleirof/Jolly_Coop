...
namespace JollyCoop
{
    class JollyCoopManager
    {
        ...
        private static string[] extraEnemyHealthStrings = new string[] { "<color=#8B4513>0</color>", "<color=#6495ED>0.1</color>", "<color=#3CB371>0.2</color>", "<color=#FF0000>0.3</color>" };
        ...
        public static float EnemyHealth
        {
            get
            {
                ...
                if (gunfig.Enabled(normalBossRewardDoubledStr))
                    result += 0.025f;
                result += extraEnemyHealthIndex * 0.1f;
                return result;
            }
        }
        ...
        internal static void InitializeGunfig()
        {
            ...
            gunfig.AddLabel(" ");
            gunfig.AddLabel(GameManager.Options.CurrentLanguage == StringTableManager.GungeonSupportedLanguages.CHINESE ?
                "对我们来说还是太简单了！".WithColor(Color.red) :
                "Still too easy for us!".WithColor(Color.red));
            gunfig.AddScrollBox(key: extraEnemyHealthStr, label: GameManager.Options.CurrentLanguage == StringTableManager.GungeonSupportedLanguages.CHINESE ?
                "额外敌人血量增量" :
